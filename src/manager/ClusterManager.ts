...
import type { Indomitable, ShardEventData } from "../Indomitable.js";
import { Delay, LibraryEvents } from "../Util.js";
import { MainWorker } from "../ipc/MainWorker.js";
/**
...
export class ClusterManager {
	...
	public async respawn(delay: number = this.manager.spawnDelay) {
		this.manager.emit(
			LibraryEvents.DEBUG,
			`Restarting Cluster ${this.id} containing [ ${this.shards.join(", ")} ] shard(s)...`,
		);
	...
	public async spawn() {
		this.manager.emit(
			LibraryEvents.DEBUG,
			`Spawning Cluster ${this.id} containing [ ${this.shards.join(", ")} ] shard(s)...`,
		);
		...
		this.worker
			.on("message", async (message) =>
				...
				this.ipc.handleRawResponse(message, (error) => this.manager.emit(LibraryEvents.ERROR, error as Error)),
			)
			...
			.once("exit", async (code, signal) => {
				...
				await this.manager.addToSpawnQueue(this);
			});
		if (!this.started) {
			this.started = true;
		}
		...
		this.manager.emit(
			LibraryEvents.DEBUG,
			`Successfully spawned Cluster ${this.id} containing [ ${this.shards.join(", ")} ] shard(s)! | Waited for cluster ready? ${this.manager.waitForReady}`,
		);
	...
	private cleanup(code: number | null, signal: string | null) {
		...
		this.manager.emit(
			LibraryEvents.DEBUG,
			`Cluster ${this.id} exited with close code ${code ?? "unknown"} signal ${signal ?? "unknown"}`,
		);
